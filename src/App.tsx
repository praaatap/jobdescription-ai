...
const App: React.FC = () => {
    ...
    useEffect(() => {
        if (isExtension && chrome.storage) {
            ...
            chrome.storage.local.get(['resumes', 'cachedResume', 'lastExtractedJob', 'jobHistory'], (result) => {
                // Load multi-resume data
                if (result.resumes) {
                    const resumeList = result.resumes as Resume[];
                    setResumes(resumeList);
                    const active = resumeList.find(r => r.isActive);
                    if (active) {
                        setActiveResumeId(active.id);
                        setResumeText(active.text);
                        setResumeFileName(active.fileName);
                        setResumeUploaded(true);
                    }
                }
                // Fallback to legacy single resume
                else if (result.cachedResume) {
                    const resume = result.cachedResume as CachedResume;
                    const migratedResume: Resume = {
                        id: '1',
                        name: 'My Resume',
                        text: resume.text,
                    ...
                    setResumes([migratedResume]);
                    setActiveResumeId('1');
                    setResumeText(resume.text);
                    setResumeFileName(resume.fileName || 'Resume');
                    setResumeUploaded(true);
                    // Migrate to new format
                    chrome.storage?.local.set({ resumes: [migratedResume] });
                }
...
Generated by JobFit AI
    ...
    const handleResumeUploadNew = async (file: File, name: string) => {
        setLoading(true);
        setError('');
        try {
            const arrayBuffer = await file.arrayBuffer();
            ...
            const updatedResumes = resumes.length === 0
                ? [newResume]
                : [...resumes, newResume];
            ...
            if (isExtension && chrome.storage) {
                chrome.storage.local.set({ resumes: updatedResumes });
            }
        } catch {
    ...
    const handleResumeSelect = (resumeId: string) => {
        ...
        const selectedResume = updatedResumes.find(r => r.id === resumeId);
        if (selectedResume) {
            setActiveResumeId(resumeId);
            setResumeText(selectedResume.text);
            setResumeFileName(selectedResume.fileName);
            setResumeUploaded(true);
        }
    ...
    const handleResumeDelete = (resumeId: string) => {
        ...
        if (activeResumeId === resumeId && updatedResumes.length > 0) {
            updatedResumes[0].isActive = true;
            setActiveResumeId(updatedResumes[0].id);
            setResumeText(updatedResumes[0].text);
            setResumeFileName(updatedResumes[0].fileName);
    ...
    const handleResumeRename = (resumeId: string, newName: string) => {
        const updatedResumes = resumes.map(r =>
    ...
    const handleJobStatusUpdate = (jobId: number, status: JobHistoryItem['status']) => {
        const updatedJobs = jobHistory.map(job =>
            job.id === jobId ? { ...job, status } : job
        );
        setJobHistory(updatedJobs);

        if (isExtension && chrome.storage) {
            chrome.storage.local.set({ jobHistory: updatedJobs });
        }
    };

    const handleJobNotesUpdate = (jobId: number, notes: string) => {
        const updatedJobs = jobHistory.map(job =>
    ...
    const handleJobDateUpdate = (jobId: number, field: 'applicationDate' | 'interviewDate', date: number) => {
        const updatedJobs = jobHistory.map(job =>
            job.id === jobId ? { ...job, [field]: date } : job
        );
        setJobHistory(updatedJobs);
                ...
                {analysisResult ? (
                    <div>
                        <AnalysisResultView
                            ...
                            onCopyKeywords={() => copyToClipboard(analysisResult.missingKeywords?.join(', ') || '')}
                            onBack={handleBackFromAnalysis}
                        />
                        <button className={styles.exportBtn} onClick={exportAnalysis} style={{ marginTop: 16 }}>
                            <Icons.Copy size={14} /> Copy Analysis Report
                        </button>
                    </div>
                ) : (
                    <>
                        ...
                        {activeTab === 'resumes' && (
                            <div className={styles.tabContent}>
                                <div className={styles.sectionTitle}><Icons.FileText size={14} /> My Resumes</div>
                                <ResumeManager
                                    resumes={resumes}
                                    onResumeSelect={handleResumeSelect}
                                    onResumeDelete={handleResumeDelete}
                                    onResumeRename={handleResumeRename}
                                    onResumeUpload={handleResumeUploadNew}
                                />
                            </div>
                        )}
                        ...
                        {activeTab === 'tracker' && (
                            <div className={styles.tabContent}>
                                <div className={styles.sectionTitle}><Icons.Briefcase size={14} /> Application Tracker</div>
                                <ApplicationTracker
                                    jobs={jobHistory}
                                    onStatusUpdate={handleJobStatusUpdate}
                                    onNotesUpdate={handleJobNotesUpdate}
                                    onDateUpdate={handleJobDateUpdate}
                            ...
                            <div className={styles.tabContent}>
                                ...
                                {keysSaved ? (
                                    <div className={styles.apiKeySection}>
                                        <div className={styles.successBox}><Icons.Check size={14} /> API key saved</div>
                                        <button className={styles.updateKeyBtn} onClick={() => { setKeysSaved(false); setApiKeyGemini(''); }}>
                                            <Icons.Settings size={14} /> Update API Key
                                        </button>
                                    </div>
                                ) : (
                                    <>
                                ...
                                <a href="https://twitter.com/itspratap9" target="_blank" rel="noopener noreferrer" className={styles.twitterBtn}>
                                    <Icons.Twitter size={14} /> Follow @itspratap9
                                </a>
